...
- **Live Terminal Logging Console:** Features an integrated monospace terminal console that captures extraction logs in real time and automatically tags them with dynamic colors (Light Blue for `INFO`, Slate Gray for `DEBUG`, Emerald Green for successful endings, and Coral Red for warnings/errors).
